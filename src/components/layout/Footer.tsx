...
const Footer = () => {
  ...
  const handleCompanyLink = (e: React.MouseEvent, item: string) => {
    e.preventDefault();
    if (item === "Case Studies") scrollToSection(e, "case-studies", "/case-studies");
        ...
        <div className="pt-10 md:pt-12 lg:pt-14">
          <div className="grid grid-cols-1 gap-9 md:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_1.2fr] lg:gap-10 xl:gap-16">
            <div className="max-w-[340px]">
              <Link to="/" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); navigate("/"); }} className="text-3xl font-semibold text-gradient-blue hover:opacity-80 transition-opacity">BR FUNNELS</Link>
              ...
              <div className="mt-7 flex items-center gap-3.5 md:mt-8 md:gap-4">
                <a href="https://www.linkedin.com/company/br-funnels" target="_blank" rel="noopener noreferrer" className="flex size-10 items-center justify-center rounded-lg border border-white/10 bg-white/4 text-white/70 transition-all hover:border-white/20 hover:bg-white/8 hover:text-white md:size-12 md:rounded-xl">
                  <LinkedInIcon className="size-5 md:size-6" />
                </a>
                <a href="#" onClick={handlePlaceholderClick} className="flex size-10 items-center justify-center rounded-lg border border-white/10 bg-white/4 text-white/70 transition-all hover:border-white/20 hover:bg-white/8 hover:text-white md:size-12 md:rounded-xl">
                  <TwitterIcon className="size-5 md:size-6" />
                </a>
                <a href="https://www.instagram.com/brfunnels1/" target="_blank" rel="noopener noreferrer" className="flex size-10 items-center justify-center rounded-lg border border-white/10 bg-white/4 text-white/70 transition-all hover:border-white/20 hover:bg-white/8 hover:text-white md:size-12 md:rounded-xl">
            ...
            <div>
              ...
              <ul className="mt-5 space-y-3.5 md:mt-7 md:space-y-5">
                {services.map(item => <li key={item}><a href="#" onClick={handlePlaceholderClick} className="text-[#B9B9B9] transition-colors hover:text-white">{item}</a></li>)}
              </ul>
            </div>
            <div>
              ...
              <ul className="mt-5 space-y-3.5 md:mt-7 md:space-y-5">
                {["Case Studies", "Process", "About", "Contact"].map(item => (
                  <li key={item}><a href="#" onClick={(e) => handleCompanyLink(e, item)} className="text-[#B9B9B9] transition-colors hover:text-white">{item}</a></li>
                ))}
              </ul>
            </div>
          ...
          <div className="mt-10 border-t border-white/8 py-5 md:mt-12 md:py-6">
            <div className="flex flex-col gap-4 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left">
              ...
              <div className="flex gap-8 text-[#B9B9B9] text-sm justify-center sm:justify-start">
                <a href="#" onClick={handlePlaceholderClick} className="transition-colors hover:text-white">Privacy Policy</a>
                <a href="#" onClick={handlePlaceholderClick} className="transition-colors hover:text-white">Terms of Service</a>
              </div>
            </div>
          </div>
        </div>
