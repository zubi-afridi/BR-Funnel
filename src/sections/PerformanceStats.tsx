      ...
      >
        {stats.map(({ id, value, title, icon }) => (
          <motion.div
            key={id}
            variants={fadeInTop}
            className="rounded-[18px] border border-white/15 bg-white/2 p-5 backdrop-blur-sm transition-all duration-300 hover:border-cyan-400/30 hover:bg-white/4"
          >
            <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl bg-[#4d4dff]/20 text-[#7c7cff]">
              {icon}
            </div>
