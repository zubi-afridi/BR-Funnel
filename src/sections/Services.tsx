    ...
    <section className="w-full px-4 sm:px-6 lg:px-8 relative z-10 pt-16 pb-12 overflow-hidden">
      <div className="mx-auto w-full max-w-[1152px]">
        ...
        <div className="text-center mb-16">
          <h2 className="text-white font-semibold tracking-[-0.02em]
                sm:text-2xl lg:text-[32px] text-xl">
            Our Revenue Growth Services
          </h2>
          <p className="mx-auto 
                mt-2 text-sm text-[#B9B9B9] sm:text-base">
            End-to-end funnel and retention marketing systems tailored to your brand.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
          {services.map((service, index) => (
            <div
              key={service.id}
              className={`relative group h-full flex flex-col rounded-[24px] overflow-hidden transition-all duration-300 ${service.active ? "p-[1px]" : "border border-white/5 border-[0.88px]"
            ...
            >
              ...
              {service.active && (
                <div className="absolute inset-0 bg-gradient-to-br from-[#793FEE] to-[#26FDFE] opacity-70"></div>
              )}

              <div
                className={`relative h-full w-full flex flex-col bg-[#0B0F1A] rounded-[21.127px] pt-10 pb-8 px-8 transition-transform duration-300 ${!service.active && "hover:bg-[#141B2D]/60 hover:border-white/10"
                  }`}
              >
                {/* Icon Box */}
                ...
                <div className="mt-8 flex items-center gap-2 group/link cursor-pointer w-fit">
                  ...
                  <div className="transition-transform duration-300 group-hover/link:translate-x-1 group-hover/link:opacity-80">
                    ...
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <defs>
                        <linearGradient id="arrow_gradient" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#793FEE" />
                          <stop offset="100%" stopColor="#26FDFE" />
                        </linearGradient>
                      </defs>
                      <path d="M2.91666 7H11.0833M11.0833 7L7.58332 3.5M11.0833 7L7.58332 10.5" stroke="url(#arrow_gradient)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                </div>
              </div>
