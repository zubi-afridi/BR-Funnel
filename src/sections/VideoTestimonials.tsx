...
const videos = [
  { name: "Sarah Johnson", role: "VP of Marketing at TechFlow Inc.", videoId: "4Pj_oHLjNUU" },
  { name: "Michael Chen", role: "Founder & CEO at GrowthLabs", videoId: "twd-acL2v1w" },
  { name: "Emily Rodriguez", role: "Head of Growth at Elevate Commerce", videoId: "XoKHw5_GSoM" }
];
  ...
  <section id="clients" className="relative z-10 w-full overflow-hidden px-4 sm:px-6 lg:px-8">
    ...
    >
      ...
      <motion.div variants={fadeInTop} className="mb-16 text-center">
        <h2 className="text-xl font-semibold tracking-[-0.02em] text-white sm:text-2xl lg:text-[32px]">Trusted by Fast-Growing Brands</h2>
        <p className="mx-auto mt-2 text-sm text-[#B9B9B9] sm:text-base">Hear directly from the founders and marketers we've helped scale</p>
      </motion.div>
      
      {/* Scroll Animation: Video Grid */}
      <motion.div 
      ...
      >
        {videos.map(({ name, role, videoId }, i) => (
          <motion.div key={i} variants={fadeInTop} className="w-full">
            ...
            <div className="pt-5">
              <h3 className="text-[17.5px] leading-relaxed font-semibold text-white">{name}</h3>
              <p className="mt-1 text-[14px] leading-relaxed text-[#B9B9B9]">{role}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
